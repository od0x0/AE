...
-add:(id)object{
	size_t index=[self find: nil];
	if(index) [self set: index-1 to: object];
	else AEListAdd(list,id,[object retain]);
	return self;
}

-remove:(id)object{
	size_t index=[self find: object];
	if(index){
		[self set: index-1 to: nil];
		[object release];
	}
	return self;
}
...
-(void)set:(size_t)index to:(id)object{
	...
	AEListAsArray(list,id)[index]=[object retain];
}
