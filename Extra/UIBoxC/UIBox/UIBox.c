...
void AEUIBoxSignalChildren(AEUIBox* root,int event,void* data){
	...
	AEUIBox** subBoxes=AEListAsArray(root->children,AEUIBox*);
	unsigned int subBoxCount=AEListLength(root->children);
	
	for(unsigned int i=0;i < subBoxCount;i++){
		AEUIBox* box=subBoxes[i];
...
void AEUIBoxRaise(AEUIBox* root,AEUIBox* box){
	...
	for(b=0;b<subBoxCount;b++){
		if(subBoxes[b]==box) break;
	}
	AEUIBox* top=subBoxes[subBoxCount-1];
	subBoxes[subBoxCount-1]=box;
	subBoxes[b]=top;
}
...
void* AEUIBoxDefaultEvent(AEUIBox* self,int event,void* data){
	switch(event){
		...
		case AEUIBoxEventAddChild:
			AEListAdd(self->children,AEUIBox*,data);
			break;
		...
		case AEUIBoxEventSDLEvent:{
			...
			switch(event->type){
				case SDL_MOUSEBUTTONDOWN:{
					float mouseY=AEContextActiveGet()->h-AEContextActiveGet()->mouse.y;
					char conditions=0;
					conditions+= self->x < AEStateActiveGet()->mouse.x;
					conditions+= self->y < mouseY;
					conditions+= (self->x+self->w) > AEStateActiveGet()->mouse.x;
