...
*/
//Most of this matrix stuff is from Alex Diener's wonderful tutorials at sacredsoftware.net
AEMathInline void AEMatrixFromIdentity(float* m){
	m[0]=1.0f;
	m[1]=0.0f;
	m[2]=0.0f;
	m[3]=0.0f;
...
AEMathInline void AEMatrixFromPerspective(float* m4x4, float fovY, float aspect, float zNear, float zFar) {
	float sine, cotangent, deltaZ;

	fovY = ((fovY*AEPiOver180) / 2.0f);
	deltaZ = (zFar - zNear);
	sine = sinf(fovY);
	if (deltaZ == 0.0f || sine == 0.0f || aspect == 0.0f) {
		return;
	}
	cotangent = (cosf(fovY) / sine);
	
	AEMatrixFromIdentity(m4x4);
	m4x4[0] = (cotangent / aspect);
	m4x4[5] = cotangent;
	m4x4[10] = (-(zFar + zNear) / deltaZ);
	m4x4[11] = -1.0f;
	m4x4[14] = ((-2.0f * zNear * zFar) / deltaZ);
	m4x4[15] = 0.0f;
}
