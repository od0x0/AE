...
static inline AEList* AEListNewWithTypeSize(size_t typeSize){
...
static inline void AEListResize(AEList* list,size_t length){
...
static inline void* AEListCheck(AEList* list){
	list->length++;
...
static inline void AEListSweep(AEList* list){
	AEListResize(list,list->length);
}
#define AEListAdd(list,type,element) ((*((type*)AEListCheck(list)))=(element))

static inline void AEListDelete(AEList* list){
	free(list->data);
	free(list);
}

static inline void AEListAddBytes(AEList* list,void* bytes){
	list->length++;
	if(list->length>list->allocated){
		list->allocated=list->length+30;
		list->data=list->data=realloc(list->data,list->allocated*list->typeSize);
...
static inline int AEList_private_memeql(char* p,char* p2,size_t size){
...
static inline size_t AEListAddBytesUnique(AEList* list,void* data){
...
static inline size_t AEListFindIndexOfBytes(AEList* list,void* bytes){
	size_t length=AEListLength(list);
	char* listdata=AEListAsArray(list,char);
...
static inline void AEListRemoveDuplicates(AEList* list,AEList* indices){
