...
static AEVBO* AEVBOFromRawMesh(AERawMesh* m,char isStatic,char hasNormals){
	...
	AEVBOAdd(vbo,NULL);
	unsigned int usages[2]={isStatic?GL_STATIC_DRAW:GL_DYNAMIC_DRAW,GL_STATIC_DRAW};
...
AEVBO* AEVBOLoad(const char* filename,int isStatic,int hasNormals){
	...
	AEVBO* vbo=AEVBOFromRawMesh(m,isStatic,hasNormals);
	AERawMeshDelete(m);
	return vbo;
}
